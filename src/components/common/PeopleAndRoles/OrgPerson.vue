            ...
            >
              ...
              <template v-if="isPerson">
                ...
                <article class="person-name">
                  ...
                  <p
                    v-if="(isExisting && isProprietor && !isFirmCorrectionFiling)"
                    class="info-text mb-0"
                  >
                    If the proprietor has changed their legal name, enter their new legal name.
                  </p>
                  <p
                    v-if="(isExisting && isPartner && !isFirmCorrectionFiling)"
                    class="info-text mb-0"
                  >
...
export default class OrgPerson extends Mixins(CommonMixin, OrgPersonMixin) {
  ...
  private validateEmailFormat (email: string): boolean {
    const VALID_FORMAT = new RegExp(/^(?=[^@]{1,64}@)[^<>()[\]\\.,;:\s@"\u0080-\uFFFF]+(\.[^<>()[\]\\.,;:\s@"\u0080-\uFFFF]+)*@(\[(25[0-5]|2[0-4][0-9]|1[0-9]{2}|[1-9][0-9]|[0-9])\.(25[0-5]|2[0-4][0-9]|1[0-9]{2}|[1-9][0-9]|[0-9])\.(25[0-5]|2[0-4][0-9]|1[0-9]{2}|[1-9][0-9]|[0-9])\.(25[0-5]|2[0-4][0-9]|1[0-9]{2}|[1-9][0-9]|[0-9])\]|([a-zA-Z0-9]([a-zA-Z0-9-]*[a-zA-Z0-9])?\.)+[a-zA-Z]{2,})$/)
